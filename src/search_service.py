...
class SearchService:
    ...
    def _is_foreign_stock(stock_code: str) -> bool:
        ...
        # 加密货币：包含 USDT、USD 或纯代码如 BTC、ETH
        if 'USDT' in upper or 'USD' in upper:
            return True
        if upper in ('BTC', 'ETH', 'BNB', 'XRP', 'SOL', 'DOGE'):
            return True
    ...
    def search_stock_news(
        ...
        is_crypto = 'USDT' in stock_code.upper() or 'USD' in stock_code.upper() or stock_code.upper() in ('BTC', 'ETH')
        ...
        if focus_keywords:
            ...
            # 加密货币使用专门的搜索关键词
            base_code = stock_code.upper().replace('USDT', '').replace('USD', '')
            query = f"{base_code} Bitcoin cryptocurrency latest news price analysis"
        elif is_foreign:
            # 港股/美股使用英文搜索关键词
