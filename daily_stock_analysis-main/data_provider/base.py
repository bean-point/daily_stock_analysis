...
def normalize_stock_code(stock_code: str) -> str:
    ...
    - 'BTCUSDT'     -> 'BTCUSDT'  (keep crypto codes as-is)
    - 'ETH-USD'     -> 'ETH-USD'  (keep crypto codes as-is)
    ...
    # 识别加密货币代码（保持原样）
    # 格式：BTCUSDT, ETHUSDT, BTC-USD, ETH-USDT 等
    if 'USDT' in upper or ('USD' in upper and len(upper) >= 6):
        return code
...
class DataFetcherManager:
    ...
    def _init_default_fetchers(self) -> None:
        ...
        crypto = CryptoFetcher()    # 加密货币数据源

        # 初始化数据源列表
        self._fetchers = [
            efinance,
            crypto,     # 加密货币数据源优先（只对加密货币有效）
            akshare,
            tushare,
            pytdx,
            baostock,
